...
async def test_official_search_reuses_cached_results(monkeypatch):
    ...
    async def fake_modrinth(query: str, **kwargs):
        calls["modrinth"] += 1
        return [
            SourceSearchResult(
                source="modrinth",
    ...
    async def fake_empty(source: str):
        async def _inner(query: str, **kwargs):
            calls[source] += 1
            return []
...
@pytest.mark.asyncio
async def test_official_search_keeps_running_when_one_source_fails(monkeypatch):
    official_server_search.clear_official_server_cache()

    async def failing_source(query: str, **kwargs):
        raise httpx.ConnectError("network unavailable")

    async def empty_source(query: str, **kwargs):
        return []

    monkeypatch.setattr(official_server_search, "search_modrinth_official_server", failing_source)
    monkeypatch.setattr(official_server_search, "search_curseforge_official_server", empty_source)
    monkeypatch.setattr(official_server_search, "search_ftb_official_server", empty_source)

    results, searched = await official_server_search.search_official_servers(
        ParsedQuery(query="ATM10", version_hint=None, source_hint=None)
    )

    assert results == []
    assert searched == ["CurseForge", "Modrinth", "FTB"]
