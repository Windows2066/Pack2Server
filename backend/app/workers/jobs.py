    ...
    try:
        ...
        task.artifacts = [
            ArtifactRead(
                ...
                + timedelta(hours=settings.artifact_retention_hours),
            )
        ]
        ...
        write_verification_report(generated.workspace_path, verification)
        refresh_server_archive(generated)

        if not verification.ok and settings.enable_startup_verification:
            task.status = "failed"
            task.progress_message = "启动验证失败"
            task.error_message = verification.message
            task.report = build_failure_report(
                "启动验证失败",
                verification.log_excerpt or [verification.message],
        ...
        report = build_success_report(
            ...
            verification_logs=verification.log_excerpt,
        )
